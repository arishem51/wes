...
Toàn bộ vòng đời của một Task được quản lý trong **một bảng duy nhất** với state machine. Các service chỉ query theo state — không có khái niệm "pool riêng biệt".

```
CREATED
  └─(validate: source point + destination trong topology)─→ RELEASEABLE
                                                                  │
                                          (Release Engine: dependency check OK — BR-PICKUP-01)
                                                                  ↓
                                                        READY_TO_ASSIGN
                                                                  │
                                          (Dispatcher: Hungarian → AGV assigned, TO gửi FMS)
                                                                  ↓
                                                             IN_FLIGHT
                                                            ↙           ↘
                                     (auto-cancel          )             ( FMS callback OK )
                                     (BR-CONFLICT-01)      )                     ↓
                                           ↓               )            PICKUP_COMPLETED
                                      RELEASEABLE          )            (→ tiếp MF-02)
                                  (dependency constraint    )
                                   được update; Release     )
                                   Engine evaluate lại)
```

**Lưu ý quan trọng:** Khi task bị auto-cancel do conflict (BR-CONFLICT-01), state quay về `RELEASEABLE` — **không phải** `READY_TO_ASSIGN` — vì dependency constraint vừa thay đổi, Release Engine cần evaluate lại từ đầu.

**Terminal states:**

| State                | Nguyên nhân                                      |
| -------------------- | ------------------------------------------------ |
| `CANCELLED`          | User/Admin delete cargo (MF-03)                  |
| `FAILED`             | AGV hoặc FMS báo lỗi                             |
| `DELIVERY_COMPLETED` | FMS callback delivery confirmed (cuối MF-02)     |

**Query pattern của từng service:**

| Service          | Query                              |
| ---------------- | ---------------------------------- |
| Release Engine   | `WHERE state = RELEASEABLE`        |
| Dispatcher       | `WHERE state = READY_TO_ASSIGN`    |
| Router           | fire ngay sau Dispatcher assign    |
| Conflict Checker | `WHERE state = IN_FLIGHT`          |
...
| 3    | WES           | Tạo Task → state `RELEASEABLE` (nếu source/destination hợp lệ) | Task chờ Release Engine xử lý             |
...
| 6    | WES → FMS     | Auto-cancel blocked tasks qua withdrawal API (nếu có conflict) | FMS callback confirmed → Task state về `RELEASEABLE` với dependency constraint mới (không phải `CANCELLED`) |
| 7    | WES           | Release Engine: query `RELEASEABLE`, dependency check OK → state `READY_TO_ASSIGN` | Tập task eligible để assign  |
| 8    | WES           | Dispatcher: query `READY_TO_ASSIGN`, Hungarian (Tasks × AGV eligible) → assign AGV → state `IN_FLIGHT` | Mỗi task được gán AGV cụ thể |
...
- **Task bị block bởi dependency:** outer chưa lấy → task giữ state `RELEASEABLE`, Release Engine re-trigger khi outer task hoàn tất.
- **Không có AGV eligible:** task giữ state `READY_TO_ASSIGN`, Dispatcher re-trigger khi AGV trở thành eligible.
